...
export default function ScheduleComponent() {
  ...
  const [showLimitAvailability, setShowLimitAvailability] = useState(formData.limitAvailability)
  const [weekCount, setWeekCount] = useState(formData.weekCount || 1)
  const [startDate, setStartDate] = useState<Date>(() => {
    const date = formData.startDate ? new Date(formData.startDate) : new Date();
    console.log("Initial start date:", date);
    return date;
  });
  const [startTime, setStartTime] = useState(formData.startTime || "5:00 AM")
  const [endDate, setEndDate] = useState<Date>(formData.endDate ? new Date(formData.endDate) : new Date())
  const [endTime, setEndTime] = useState(formData.endTime || "11:59 PM")

  // Update coupon context whenever schedule values change
  useEffect(() => {
    setFormData((prev) => ({
      ...prev,
      startDate: startDate.toISOString(),
      startTime: startTime,
      endDate: showEndDate ? endDate.toISOString() : "",
      endTime: showEndDate ? endTime : "",
      limitAvailability: showLimitAvailability,
      weekCount: weekCount,
      // Make sure to update these when they change in your component
      selectedWeekdays: prev.selectedWeekdays, // Update this when weekdays change
      availabilityStartTime: prev.availabilityStartTime, // Update when changed
      availabilityEndTime: prev.availabilityEndTime, // Update when changed
    }));
  }, [startDate, startTime, endDate, endTime, showEndDate, showLimitAvailability, weekCount, setFormData]);
  ...
  const handleRemoveLimitAvailability = () => {
    ...
    // Clear limit availability settings in form data
    setFormData((prev) => ({
      ...prev,
    ...
    <div className="space-y-4">
          ...
          <div className="relative">
            <select
              ...
              onChange={(e) => {
                ...
                setFormData((prev) => ({
                  ...prev,
                  selectedWeekdays: [...(prev.selectedWeekdays || []), selectedWeekday],
                }))
              }}
            >
      ...
      {showLimitAvailability && (
        <div className="flex flex-wrap items-center gap-2 pl-6">
          ...
          <button
            type="button"
            onClick={handleRemoveLimitAvailability}
